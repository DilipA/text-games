...
if __name__ == "__main__":
    ...
    numEpisode = 0
    numStep = 0
    stateVocab = dict_wordId.keys()
    actionVocab = dict_actionId.keys()
    print 'Initial state vocab size = {0}'.format(len(stateVocab))
    print 'Initial action vocab size = {0}'.format(len(actionVocab))
    ...
    while numEpisode < 100000:
        ...
        state = parse_sentence(text)
        actions = [parse_sentence(a) for a in actions]
        for w in state:
            if w not in stateVocab:
                stateVocab.append(w)
                newly_added_state.append(w)
                vocab_updated = True

        for a in actions:
            for w in a:
                if w not in actionVocab:
                    actionVocab.append(w)
                    newly_added_action.append(w)
                    vocab_updated = True
        ...
        if len(actions) == 0 or numStep > 250:
            ...
            if not vocab_updated:
                ...
                if episode_count == 10000:
                    new_word_dict = {word:index for index,word in enumerate(stateVocab)}
                    new_action_dict = {word:index for index, word in enumerate(actionVocab)}
                    with open(os.path.join(curDirectory, "machineofdeath_wordId2.pickle"), "w") as outfile:
                        pickle.dump(new_word_dict, outfile)
                    with open(os.path.join(curDirectory, "machineofdeath_actionId2.pickle"), "w") as outfile:
                        pickle.dump(new_action_dict, outfile)
                    print 'Final state vocab size = {0}'.format(len(stateVocab))
                    print 'Final action vocab size = {0}'.format(len(actionVocab))
                    print 'Newly added state vocab words: {0}'.format(newly_added_state)
                    print 'Newly added action vocab words: {0}'.format(newly_added_action)
                    break
            ...
            mySimulator.Restart()
            numEpisode += 1
            print 'Finished episode {0} with {1} steps'.format(numEpisode+1, numStep)
            numStep = 0
        else:
            terminal = False
            playerInput = np.random.choice(len(actions))
